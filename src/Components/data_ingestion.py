...
class DataIngestion:
    ...
    def initiate_data_ingestion(self):
        ...
        try:
            ...
            df.to_csv(self.ingestion_config_data.Row_data_path,index=False,header=True)
            logging.info("Data read completely")

            #train and test model

            Train_data,test_data=train_test_split(df,random_state=42,test_size=0.2)
            Train_data.to_csv(self.ingestion_config_data.train_data_path,index=False,header=True)
            test_data.to_csv(self.ingestion_config_data.test_data_path,index=False,header=True)


            return (

                self.ingestion_config_data.train_data_path,
                self.ingestion_config_data.test_data_path,
            )
